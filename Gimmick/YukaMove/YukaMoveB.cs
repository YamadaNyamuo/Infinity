...
public class YukaMoveB : MonoBehaviour {
    ...
    void OnCompleteHandler()
    {
        ...
        if (type)
        {
            if (cnt == 0 || cnt == iTweenPath.GetPath(PathName).Length - 1)
            {
                cntPlus *= (-1);
            }
        }
    ...
    void Update()
    {
        ...
        {
            if (!iTweenMoving && cnt < iTweenPath.GetPath(PathName).Length)
            {
                if (type)
                {
                    typeString = "looptype";
                    iTween.MoveTo(this.gameObject, iTween.Hash("position", iTweenPath.GetPath(PathName)[cnt], "time", time[cnt], "oncomplete", "OnCompleteHandler", "easeType", easeType, typeString, loopType));
                    iTweenMoving = true;
                ...
                {
                    typeString = "easeType";
                    iTween.MoveTo(this.gameObject, iTween.Hash("position", iTweenPath.GetPath(PathName)[cnt], "time", time[cnt], "oncomplete", "OnCompleteHandler", "easeType", easeType));
                    iTweenMoving = true;
